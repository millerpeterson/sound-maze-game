...
Room.method('passageTo', function(otherRoom) {
  return _.any(this.passages, function(r) {
    return r.pos.isEq(otherRoom.pos);
  });
});

Room.method('accessibleNeighbors', function() {
  var that = this;
  return _.filter(this.neighbors(), function(n) {
    return that.passageTo(n);
  });
});
